...
import { fiveLaunchFactory, safeLaunchTemplate } from "@/Constants/config"
...
export default function Factory(): JSX.Element {
  ...
  useEffect(() => {
    if (chain && chain.id) {
      const factoryAddress = fiveLaunchFactory[chain.id] || ""
  ...
  const { data: deployFee, error: readError } = useContractRead({
    address: fiveLaunchFactory[chainId] as `0x${string}`,
    abi: fiveFactoryABI,
