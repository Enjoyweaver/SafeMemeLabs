...
const ProfilePage: React.FC = () => {
  ...
  const handleWalletClick = async (wallet: WalletInfo) => {
    ...
    if (!arweaveWallet) {
      alert("Please connect your browser wallet for free transactions.")
    }
  }
  ...
  const handleSaveProfileForJwkWallet = async () => {
    try {
      const data = JSON.stringify({
        handleName: profileData.handleName || "",
        name: profileData.name || "",
  ...
  const handleImageUploadForJwkWallet = async () => {
    if (selectedImage) {
      try {
        const reader = new FileReader()
        reader.onload = async (event) => {
          ...
          if (data) {
            ...
            if (walletInfo?.jwk) {
              // Create the transaction with the key
              const transaction = await arweave.createTransaction(
                { data: dataArray },
                walletInfo.jwk
              )
          ...
          <>
            <div className="profile-image">
              ...
              {selectedImage && (
                <>
                  {walletInfo?.jwk ? (
                    <button onClick={handleImageUploadForJwkWallet}>
            ...
            <div className="profile-wallets">
              ...
              />
              {/* Add inputs for other wallet types */}
              {/* ... */}
            </div>
            {walletInfo?.jwk ? (
              <button onClick={handleSaveProfileForJwkWallet}>
                Save Profile
              </button>
            ) : (
